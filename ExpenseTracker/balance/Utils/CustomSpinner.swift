...
//  Copyright © 2020 First Image. All rights reserved.
//
...
@IBDesignable class CustomSpinner : UIView, UITableViewDelegate, UITableViewDataSource {

    
    fileprivate var lastSuperView: UIView?
    fileprivate var firstDraw: Bool = true
    
    @IBInspectable var heightTableviewCell: CGFloat = 30{didSet {updateUI()}}
    ...
    //Drop down list
    @IBInspectable var dDLMaxSize: CGFloat = 240
    ...
    @IBInspectable var dDLblurEnable: Bool = false
    
    @IBInspectable var imageSelect : UIImage?
    var imageSelected:UIImage { if #available(iOS 13.0, *) {
        return imageSelect ?? UIImage(systemName: "heart.fill")!
    } else {
        return UIImage()
        } }
    
    var delegate:CustomSpinnerDelegate!

    //actual seleted index
    fileprivate(set) internal var selectedIndex = INDEX_NOTHING

    fileprivate var labelValue: UILabel!
    fileprivate var imageValue : UIImageView!
    ...
    fileprivate func initCustomView() {
       // backgroundColor = UIColor.clear  // clear black background
        ...
        //Open spinner click
        let gesture = UITapGestureRecognizer(target: self, action: #selector(CustomSpinner.openSpinner(_:)))
    ...
    override func draw(_ rect: CGRect) {
        ...
        if firstDraw {
            ...
            let vSize = frame.height * 0.75
            let imageView = UIImageView(frame: CGRect(x: frame.width - 10 - vSize / 4 - tWidth / sqrt(2/3), y: frame.origin.y + frame.height / 2  - vSize / 2 , width: vSize, height: vSize))
            imageView.image = imageSelected
            //imageView.translatesAutoresizingMaskIntoConstraints = false
            //let image = UIImage(named: "imgPrueba")
    ...
    @objc func openSpinner(_ sender:UITapGestureRecognizer){
        ...
        tableviewChoose.alwaysBounceVertical = false
        ...
        tableviewChoose.rowHeight = heightTableviewCell
        //Show stroke
        if(dDLStroke) {
        ...
        let gesture = UITapGestureRecognizer(target: self, action: #selector(CustomSpinner.closeSpinner))
    ...
    fileprivate func drawCanvas(frame: CGRect = CGRect(x: 0, y: 0, width: 86, height: 11)) {
        ...
        case SpinnerType.type.rawValue :
...
protocol CustomSpinnerDelegate{
